...
const Appointments = () => {
  ...
  const getAppointments = async () => {
    ...
    try {
      const response = await axios.get(`${API_ENDPOINTS.GET_APPOINTMENTS_BY_COUNCSELLOR_ID}/${user.id}`);
  ...
  const toggleMenu = (id, appt, mode) => {
    ...
    if (mode === "note") {
      setSelectedNotes(appt?.notes?.text || "")
  ...
  const setUpdatedNotes = (notes, mode) => {
    if (mode === "note") {
      addNotes(notes, selectedAppointment);
    }
  ...
  const addNotes = async (notes, appointment) => {
    addToast("Adding Notes...", "info");
    try {
      const response = await axios.put(`${API_ENDPOINTS.APPOINTMENT}/${appointment._id}/notes`, {
        text: notes,
        attachments: "url_123",
      ...
      addToast(error.response?.data?.message || "Failed to add notes. Please try again.", "error");
      throw error;
    } finally {
      // setLoading(false);
    }
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto">
      ...
      {showNotesDialog && <AddNotes selectedNotes = {selectedNotes} setUpdatedNotes={setUpdatedNotes} />}
              ...
              <div key={appt._id} className="bg-white rounded-xl border border-gray-200 overflow-hidden shadow-sm">
                ...
                <div className="bg-gray-50 px-5 py-6 flex justify-between items-start relative">
                  <div className="flex items-center space-x-3">
                    <div className={`w-12 h-12 rounded-xl flex items-center justify-center text-sm font-medium ${getRandomColor(appt.clientId.name[0])}`}>{appt.clientId.name[0].toUpperCase()}</div>
                    <div>
                      <h3
                      ...
                      <p className="text-sm text-gray-500">{"Platform User"}</p>
                    </div>
                  </div>
                    ...
                    >
                      ...
                      {openMenuId === appt._id && (
                        <div className=" absolute right-[15px] top-[30px] mt-1 w-36 bg-white border border-gray-200 rounded-md shadow-md z-10">
                          {user.role === "counselor" && (
                            <button onClick={() => toggleMenu(appt._id, appt, "note")} className="block w-full px-4 py-2 text-[14px] text-gray-500 hover:bg-gray-100">
                              Add Notes
                            </button>
                          )}
